		...
		openModal(status, item, index){
			switch(status){
				...
				case 'editProduct':
					...
					const apiUrl =`${this.api.path}${this.api.uuid}/admin/ec/product/${item.id}`;
					// console.log(item.id);
					axios.get(apiUrl).then(response =>{
						// console.log(response)
						this.editProduct = response.data.data; // 將點擊的產品複製一份出來加入物件
						$('#productModal').modal('show');
						this.loadingBtn = '';// 清除
					});
					break;
